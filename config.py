...
class filter_t(metric):
	def __init__(self, below_discard, below_discard_hq, contribution=0, power_mean=-1000):
		super().__init__([below_discard]*3, [below_discard_hq]*3, contribution, power_mean)
class target_t(metric):
	def __init__(self, target, target_hq, contribution=1, power_mean=-1000):
...
class CONFIG:
	...
	PSNR        = filter_t(30, 56)
	SSIM_DB     = filter_t(17, 25)
	PSNR_HVS_CB = filter_t(30, 56)
